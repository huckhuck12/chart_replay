...
### v2026-08-20 持仓操作与数据源增强

- **止盈止损按钮移至图表上**
  开仓后，图表右上角悬浮「设止损 / 设止盈」按钮，已设置时直接标注当前价位（如 `止损 2500.00`）；侧栏抽屉仅保留 SL/TP 数值展示，操作入口收拢到图表区。
- **交易对下拉框默认收起**
  `renderPairDropdown` 默认不再自动展开，仅在用户主动聚焦/点击搜索框或切换分类时弹出，避免每次打开页面的抖动。
- **数据加载多源镜像与代理兜底**
  加密货币新增官方无地域限制镜像 `data-api.binance.vision`（优先），主端点 `api.binance.com` 被区域封锁(451)时自动切换；Yahoo Finance 增加 `query1/query2` 双主机。直连全失败时，再按 `corsproxy.io` → `allorigins` 双公共代理兜底（含 `file://` 直开）。超时与真实业务错误（如 404）不降级，避免误用模拟数据。
